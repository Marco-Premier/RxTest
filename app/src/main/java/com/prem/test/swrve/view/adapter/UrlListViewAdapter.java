...
public class UrlListViewAdapter extends RealmRecyclerViewAdapter<SearchHistoryStore,UrlListViewHolder> {


    private View.OnClickListener onDownloadImageListener;
    private View.OnClickListener onOpenImageListener;
    private List<SearchHistoryStore> urls;
    private DownloadImageState downloadImageState;
    @Inject Context context;

    public UrlListViewAdapter(OrderedRealmCollection<SearchHistoryStore> urls, View.OnClickListener onDownloadImageListener, View.OnClickListener onOpenImageListener, DownloadImageState downloadImageState){
        super(urls,true);
    ...
    public void refreshStore(List<SearchHistoryStore> urls){
        this.urls = urls;
    ...
    public void onBindViewHolder(UrlListViewHolder urlListViewHolder, int position) {
        ...
        final SearchHistoryStore searchHistoryStore = this.urls.get(position);
        String urlPath = searchHistoryStore.getUrl();
        try {
            final URL url = new URL(searchHistoryStore.getUrl());
            urlPath = "..." + url.getFile();
        }catch (MalformedURLException exception){}
        DateFormat df = new SimpleDateFormat("dd/MM/yyyy hh:mm:ss");
        urlListViewHolder.rlWrapperUrl.setTag(searchHistoryStore.getIdUrl());
        urlListViewHolder.tvUrl.setText(urlPath);
        urlListViewHolder.tvDateTime.setText(df.format(searchHistoryStore.getDownloadedAt()));

        hideButtons(urlListViewHolder);

        if (downloadImageState.getRequestStatus().equals(REQUEST_STATUS.IN_FLIGHT.toString()) && downloadImageState.getCurrentIdUrl().equals(searchHistoryStore.getIdUrl())) {
            ...
            if (downloadImageState.getRequestStatus().equals(REQUEST_STATUS.FAILURE.toString()) && downloadImageState.getCurrentIdUrl().equals(searchHistoryStore.getIdUrl())) {
            ...
            }else if (null != downloadImageState.getCurrentIdUrl() && downloadImageState.getCurrentIdUrl().equals(searchHistoryStore.getIdUrl())){
